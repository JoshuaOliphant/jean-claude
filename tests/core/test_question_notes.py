# ABOUTME: Tests for question note handler in NotesProjectionBuilder
# ABOUTME: Tests apply_agent_note_question() method to handle question note events and add to notes list

"""Tests for question note handler in NotesProjectionBuilder.

This test module validates the implementation of apply_agent_note_question()
method that handles agent.note.question events and properly adds them to
...
class TestQuestionNoteHandler:
    """Test the apply_agent_note_question method."""

    def test_adds_question_note_to_empty_state(self):
        """Test that question note is properly added to empty notes state."""
        builder = NotesProjectionBuilder()
        initial_state = builder.get_initial_state()

        # Create question event data matching AgentNoteEventData schema
        event_data = {
            "agent_id": "agent-123",
            "title": "Test Question",
            "content": "What is the best approach for implementing this feature?",
            "tags": ["implementation", "approach"],
            "related_file": "src/main.py",
            "related_feature": "feature-456"
        }

        result_state = builder.apply_agent_note_question(event_data, initial_state)
        ...
        assert note['title'] == "Test Question"
        assert note['content'] == "What is the best approach for implementing this feature?"
        assert note['category'] == "question"
        assert note['tags'] == ["implementation", "approach"]
        assert note['related_file'] == "src/main.py"
        assert note['related_feature'] == "feature-456"
        assert 'created_at' in note

    def test_adds_question_note_with_minimal_data(self):
        """Test question note with only required fields."""
        builder = NotesProjectionBuilder()
        initial_state = builder.get_initial_state()
        ...
        assert note['title'] == "Minimal Question"
        assert note['content'] == "How should this work?"
        assert note['category'] == "question"
        assert note['tags'] == []
        assert note['related_file'] is None
        assert note['related_feature'] is None

    def test_adds_question_note_to_existing_notes(self):
        """Test adding question note when notes already exist."""
        builder = NotesProjectionBuilder()
        ...
        result_state = builder.apply_agent_note_question(event_data, existing_state)
        ...
        assert new_note['title'] == "New Question"
        assert new_note['category'] == "question"

    def test_updates_by_category_index(self):
        """Test that by_category index is updated when adding question note."""
        builder = NotesProjectionBuilder()
        initial_state = builder.get_initial_state()
        ...
        assert 'question' in result_state['by_category']
        assert result_state['by_category']['question'] == [0]

    def test_updates_by_agent_index(self):
        """Test that by_agent index is updated when adding question note."""
        builder = NotesProjectionBuilder()
        initial_state = builder.get_initial_state()
    ...
    def test_updates_by_tag_index(self):
        """Test that by_tag index is updated when adding question note."""
        builder = NotesProjectionBuilder()
        initial_state = builder.get_initial_state()
        ...
        event_data = {
            ...
            "content": "How to handle multiple tags?",
    ...
    def test_handles_empty_tags_list(self):
        """Test question note with empty tags list doesn't break indexing."""
        builder = NotesProjectionBuilder()
        initial_state = builder.get_initial_state()
        ...
        event_data = {
            ...
            "content": "How to handle this without tags?",
            "tags": []
        }
    ...
    def test_preserves_original_state_immutability(self):
        ...
        event_data = {
            "agent_id": "agent-123",
            "title": "Test Question",
            "content": "How to test immutability?",
            "tags": ["test"]
        }

        result_state = builder.apply_agent_note_question(event_data, original_state)
    ...
    def test_updates_existing_indexes_correctly(self):
        ...
        # by_category should have both decision and question
        assert set(result_state['by_category'].keys()) == {'decision', 'question'}
        assert result_state['by_category']['decision'] == [0]
        assert result_state['by_category']['question'] == [1]
    ...
    def test_note_contains_timestamp(self):
        """Test that question notes include a created_at timestamp."""
        builder = NotesProjectionBuilder()
        initial_state = builder.get_initial_state()
        ...
        event_data = {
            ...
            "content": "When should this be completed?",
            "tags": ["timestamp"]
        }
...
class TestQuestionNoteHandlerEdgeCases:
    """Test edge cases and error conditions for question note handler."""
    ...
    def test_handles_malformed_state_gracefully(self):
        ...
        event_data = {
            "agent_id": "agent-123",
            "title": "Test Question",
            "content": "How to handle missing keys?",
            "tags": ["test"]
        }

        # Should handle gracefully and complete missing indexes
        result_state = builder.apply_agent_note_question(event_data, incomplete_state)
    ...
    def test_preserves_existing_state_structure(self):
        ...
        event_data = {
            "agent_id": "agent-123",
            "title": "Test Question",
            "content": "Will custom fields be preserved?",
            "tags": ["test"]
        }

        result_state = builder.apply_agent_note_question(event_data, state_with_extras)
