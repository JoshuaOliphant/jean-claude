# ABOUTME: Tests for reflection note handler in NotesProjectionBuilder
# ABOUTME: Tests apply_agent_note_reflection() method to handle reflection note events and add to notes list

"""Tests for reflection note handler in NotesProjectionBuilder.

This test module validates the implementation of apply_agent_note_reflection()
method that handles agent.note.reflection events and properly adds them to
...
class TestReflectionNoteHandler:
    """Test the apply_agent_note_reflection method."""

    def test_adds_reflection_note_to_empty_state(self):
        """Test that reflection note is properly added to empty notes state."""
        builder = NotesProjectionBuilder()
        initial_state = builder.get_initial_state()

        # Create reflection event data matching AgentNoteEventData schema
        event_data = {
            "agent_id": "agent-123",
            "title": "Test Reflection",
            "content": "This is a thoughtful reflection about the process.",
            "tags": ["process", "reflection"],
            "related_file": "src/main.py",
            "related_feature": "feature-456"
        }

        result_state = builder.apply_agent_note_reflection(event_data, initial_state)
        ...
        assert note['title'] == "Test Reflection"
        assert note['content'] == "This is a thoughtful reflection about the process."
        assert note['category'] == "reflection"
        assert note['tags'] == ["process", "reflection"]
        assert note['related_file'] == "src/main.py"
        assert note['related_feature'] == "feature-456"
        assert 'created_at' in note

    def test_adds_reflection_note_with_minimal_data(self):
        """Test reflection note with only required fields."""
        builder = NotesProjectionBuilder()
        initial_state = builder.get_initial_state()
        ...
        assert note['title'] == "Minimal Reflection"
        assert note['content'] == "Simple reflection."
        assert note['category'] == "reflection"
        assert note['tags'] == []
        assert note['related_file'] is None
        assert note['related_feature'] is None

    def test_adds_reflection_note_to_existing_notes(self):
        """Test adding reflection note when notes already exist."""
        builder = NotesProjectionBuilder()
        ...
        event_data = {
            ...
            "content": "Fresh reflection content.",
            "tags": ["new", "important"]
        }

        result_state = builder.apply_agent_note_reflection(event_data, existing_state)
        ...
        assert new_note['title'] == "New Reflection"
        assert new_note['category'] == "reflection"

    def test_updates_by_category_index(self):
        """Test that by_category index is updated when adding reflection note."""
        builder = NotesProjectionBuilder()
        initial_state = builder.get_initial_state()
        ...
        assert 'reflection' in result_state['by_category']
        assert result_state['by_category']['reflection'] == [0]

    def test_updates_by_agent_index(self):
        """Test that by_agent index is updated when adding reflection note."""
        builder = NotesProjectionBuilder()
        initial_state = builder.get_initial_state()
    ...
    def test_updates_by_tag_index(self):
        """Test that by_tag index is updated when adding reflection note."""
        builder = NotesProjectionBuilder()
        initial_state = builder.get_initial_state()
    ...
    def test_handles_empty_tags_list(self):
        """Test reflection note with empty tags list doesn't break indexing."""
        builder = NotesProjectionBuilder()
        initial_state = builder.get_initial_state()
    ...
    def test_preserves_original_state_immutability(self):
        ...
        result_state = builder.apply_agent_note_reflection(event_data, original_state)
    ...
    def test_updates_existing_indexes_correctly(self):
        ...
        # by_category should have both decision and reflection
        assert set(result_state['by_category'].keys()) == {'decision', 'reflection'}
        assert result_state['by_category']['decision'] == [0]
        assert result_state['by_category']['reflection'] == [1]
    ...
    def test_note_contains_timestamp(self):
        """Test that reflection notes include a created_at timestamp."""
        builder = NotesProjectionBuilder()
        initial_state = builder.get_initial_state()
...
class TestReflectionNoteHandlerEdgeCases:
    """Test edge cases and error conditions for reflection note handler."""
    ...
    def test_handles_malformed_state_gracefully(self):
        ...
        result_state = builder.apply_agent_note_reflection(event_data, incomplete_state)
    ...
    def test_preserves_existing_state_structure(self):
        ...
        result_state = builder.apply_agent_note_reflection(event_data, state_with_extras)
