# ABOUTME: Tests for todo note handler in NotesProjectionBuilder
# ABOUTME: Tests apply_agent_note_todo() method to handle todo note events and add to notes list

"""Tests for todo note handler in NotesProjectionBuilder.

This test module validates the implementation of apply_agent_note_todo()
method that handles agent.note.todo events and properly adds them to
...
class TestTodoNoteHandler:
    """Test the apply_agent_note_todo method."""

    def test_adds_todo_note_to_empty_state(self):
        """Test that todo note is properly added to empty notes state."""
        builder = NotesProjectionBuilder()
        initial_state = builder.get_initial_state()

        # Create todo event data matching AgentNoteEventData schema
        event_data = {
            "agent_id": "agent-123",
            "title": "Test Todo",
            "content": "This is a todo task that needs to be completed.",
            "tags": ["task", "todo"],
            "related_file": "src/core/base.py",
            "related_feature": "feature-789"
        }

        result_state = builder.apply_agent_note_todo(event_data, initial_state)
        ...
        assert note['title'] == "Test Todo"
        assert note['content'] == "This is a todo task that needs to be completed."
        assert note['category'] == "todo"
        assert note['tags'] == ["task", "todo"]
        assert note['related_file'] == "src/core/base.py"
        assert note['related_feature'] == "feature-789"
        assert 'created_at' in note

    def test_adds_todo_note_with_minimal_data(self):
        """Test todo note with only required fields."""
        builder = NotesProjectionBuilder()
        initial_state = builder.get_initial_state()
        ...
        assert note['title'] == "Key Todo Task"
        assert note['content'] == "Important task to complete."
        assert note['category'] == "todo"
        assert note['tags'] == []
        assert note['related_file'] is None
        assert note['related_feature'] is None

    def test_adds_todo_note_to_existing_notes(self):
        """Test adding todo note when notes already exist."""
        builder = NotesProjectionBuilder()
        ...
        event_data = {
            "agent_id": "agent-456",
            "title": "New Todo",
            "content": "Fresh todo content.",
            "tags": ["important", "urgent"]
        }

        result_state = builder.apply_agent_note_todo(event_data, existing_state)
        ...
        assert new_note['title'] == "New Todo"
        assert new_note['category'] == "todo"

    def test_updates_by_category_index(self):
        """Test that by_category index is updated when adding todo note."""
        builder = NotesProjectionBuilder()
        initial_state = builder.get_initial_state()
        ...
        assert 'todo' in result_state['by_category']
        assert result_state['by_category']['todo'] == [0]

    def test_updates_by_agent_index(self):
        """Test that by_agent index is updated when adding todo note."""
        builder = NotesProjectionBuilder()
        initial_state = builder.get_initial_state()
    ...
    def test_updates_by_tag_index(self):
        """Test that by_tag index is updated when adding todo note."""
        builder = NotesProjectionBuilder()
        initial_state = builder.get_initial_state()
        ...
        event_data = {
            ...
            "title": "Tagged Todo",
            "content": "Test content.",
            "tags": ["urgent", "feature", "implementation"]
        }
        ...
        assert 'feature' in result_state['by_tag']
        assert result_state['by_tag']['feature'] == [0]
        assert 'implementation' in result_state['by_tag']
        assert result_state['by_tag']['implementation'] == [0]

    def test_handles_empty_tags_list(self):
        """Test todo note with empty tags list doesn't break indexing."""
        builder = NotesProjectionBuilder()
        initial_state = builder.get_initial_state()
    ...
    def test_preserves_original_state_immutability(self):
        ...
        result_state = builder.apply_agent_note_todo(event_data, original_state)
    ...
    def test_updates_existing_indexes_correctly(self):
        """Test that existing indexes are properly updated when adding new todo notes."""
        builder = NotesProjectionBuilder()
        ...
        # by_category should have both observation and todo
        assert set(result_state['by_category'].keys()) == {'observation', 'todo'}
        assert result_state['by_category']['observation'] == [0]
        assert result_state['by_category']['todo'] == [1]
    ...
    def test_note_contains_timestamp(self):
        """Test that todo notes include a created_at timestamp."""
        builder = NotesProjectionBuilder()
        initial_state = builder.get_initial_state()
...
class TestTodoNoteHandlerEdgeCases:
    """Test edge cases and error conditions for todo note handler."""
    ...
    def test_handles_malformed_state_gracefully(self):
        ...
        result_state = builder.apply_agent_note_todo(event_data, incomplete_state)
    ...
    def test_preserves_existing_state_structure(self):
        ...
        result_state = builder.apply_agent_note_todo(event_data, state_with_extras)
